...
struct VertexOutputForwardAdd
{
	...
	half4 redLight_data0 : TEXCOORD6;
	half4 redLight_data1 : TEXCOORD9;
	half4 redLight_data2 : TEXCOORD10;
#endif
...
half4 fragForwardAddInternal (VertexOutputForwardAdd i)
{
	...
	half atten, specularAttenuation;
	half3 specLightColor;
	
	REDLIGHT_EARLY_OUT_(i.worldPos.xyz);
	REDLIGHT_FRAGMENT_SETUP;
	
	float range = i.redLight_data2.w;
	
	// proxy function for light types
	redLightFunction(_LightTexture0, i.worldPos.xyz, s.normalWorld, s.eyeVec, s.oneMinusRoughness, 1.0 - s.oneMinusReflectivity, 
						light, atten, specularAttenuation, specLightColor, redLight, range);
	
	half4 c = REDPLANT_BRDF_PBS (s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, -s.eyeVec, 
									light, noIndirect, specularAttenuation, specLightColor);
